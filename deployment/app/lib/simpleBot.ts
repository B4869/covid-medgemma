const txt = `
ขออภัยครับ ตอนนี้ยังไม่สามารถใช้ AI จริง (LLM 4B parameters) บนเว็บนี้ได้

**เหตุผล:**
- โมเดล AI ที่ใช้ต้องอาศัย GPU และ RAM สูงมาก
- Vercel เป็น Serverless จึงไม่มี GPU และหน่วยความจำไม่พอรันโมเดลขนาดใหญ่

เดโมนี้จึงเป็นแบบ Rule-based (ตอบตามกฎที่ตั้งไว้ล่วงหน้า) เท่านั้น

หากอยากลองใช้ AI จริงเต็มประสิทธิภาพ กรุณาดาวน์โหลดโปรเจกต์แล้วรันบนเครื่องตัวเอง (Local + GPU)

**วิธีการติดตั้งโปรเจกต์ (แบบละเอียด):**
https://howto.run/
